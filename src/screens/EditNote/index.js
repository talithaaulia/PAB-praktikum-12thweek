...
import { 
  Alert, Box, Heading, Text, FormControl, 
  Modal, ModalBackdrop, AlertText, Fab, FabLabel, 
  FabIcon, ArrowLeftIcon, ScrollView 
  ...
  return (
    <Box flex={1} backgroundColor="$white" justifyContent="center">
      ...
      <Box shadowColor="$black" shadowOffset={{ width: 0, height: 2 }} shadowOpacity={"$25"} 
           shadowRadius={"$3.5"} elevation={"$5"} backgroundColor="$white" borderRadius={"$md"} 
           mt={"$8"} mx={"$3"} px={"$3"} pt={"$2"}
      >
        ...
        <FormControl>
          ...
          <Input 
            textarea={true} label="Content" width={"$full"} height={"$32"} value={content} 
            onChangeText={(content) => setContent(content)} 
          />
          <Pilihan label="Status" value={status} selectedValue={status} onValueChange={(status) => ubahStatus(status)} />
          <Pilihan 
            label="Category" selectedValue={category} datas={categoryUser} 
